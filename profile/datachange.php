<!doctype html>
<html class="no-js h-100" lang="en">

<head>
  <meta charset="utf-8">
  <meta http-equiv="x-ua-compatible" content="ie=edge">
  <title>PeachSignage</title>
  <meta name="description" content="A high-quality &amp; free Bootstrap admin dashboard template pack that comes with lots of templates and components.">
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  <link rel="icon" href="../images/icon/48x48.png">
  <link href="https://use.fontawesome.com/releases/v5.0.6/css/all.css" rel="stylesheet">
  <link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">
  <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
  <link rel="stylesheet" id="main-stylesheet" data-version="1.0.0" href="../styles/shards-dashboards.1.0.0.min.css">
  <link rel="stylesheet" href="../styles/extras.1.0.0.min.css">
  <script async defer src="https://buttons.github.io/buttons.js"></script>
</head>

<body class="h-100">

  <div class="container-fluid">
    <div class="row">
      <!-- Main Sidebar -->
      <aside class="main-sidebar col-12 col-md-3 col-lg-2 px-0" style="background-color: #24282d">
        <div class="main-navbar">
          <!-- <nav class="navbar align-items-stretch navbar-light bg-white flex-md-nowrap border-bottom p-0">
            <a class="navbar-brand w-100 mr-0" href="#" style="line-height: 25px; background-color: #24282d">

              </a> -->
            <a class="toggle-sidebar d-sm-inline d-md-none d-lg-none">
                <i class="material-icons">&#xE5C4;</i>
              </a>
          <!-- </nav> -->
        </div>

        <div class="nav-wrapper">
          <ul class="nav flex-column">
            <li class="nav-item" style="background-color: #24282d">
              <a class="nav-link active" style="background-color: #24282d">
                  <span style="color: #eeeeee">การเเสดงผลบนอุปกรณ์</span>
                  <a class="dropdown-item" href="../showdisplay/show.php" style="margin-left: 30px; color: #eeeeee; background-color: #24282d">การเเสดงผล</a>
                  <a class="dropdown-item" href="../showdisplay/selectshop.php" style="margin-left: 30px; color: #eeeeee; background-color: #24282d">เลือกร้านค้า</a>
              </a>
            </li>
            <li class="nav-item" style="background-color: #24282d">
              <a class="nav-link active" style="background-color: #24282d">
                  <span style="color: #eeeeee">การจัดการอุปกรณ์</span>
                  <a class="dropdown-item" href="../statusdeviece/devicestatus.php" style="margin-left: 30px; color: #eeeeee; background-color: #24282d">สถานะอุปกรณ์</a>
                  <a class="dropdown-item" href="../statusdeviece/selectdevice.php" style="margin-left: 30px; color: #eeeeee; background-color: #24282d">เลือกอุปกรณ์</a>
              </a>
            </li>
            <li class="nav-item" style="background-color: #24282d">
              <a class="nav-link active" style="background-color: #24282d">
                  <span style="color: #eeeeee">การจัดการไฟล์</span>
                  <a class="dropdown-item" href="../contentsmanagement/video.php" style="margin-left: 30px; color: #eeeeee; background-color: #24282d">การจัดการวีดีโอ</a>
              <a class="dropdown-item" href="../contentsmanagement/pic.php" style="margin-left: 30px; color: #eeeeee; background-color: #24282d">การจัดการภาพ</a>
              <a class="dropdown-item" href="../contentsmanagement/text.php" style="margin-left: 30px; color: #eeeeee; background-color: #24282d">การจัดการข้อความ</a>
              <a class="dropdown-item" href="../contentsmanagement/website.php" style="margin-left: 30px; color: #eeeeee; background-color: #24282d">การจัดการเว็บไซต์</a>
              </a>
            </li>
            <li class="nav-item" style="background-color: #24282d">
              <a class="nav-link active" style="background-color: #24282d">
                  <span style="color: #eeeeee">การจัดการข้อมูลผู้ใช้งาน</span>
                  <a class="dropdown-item" href="../profile/datachange.php" style="margin-left: 30px; color: #eeeeee; background-color: #24282d">เเก้ไขข้อมูลส่วนตัว</a>
              <a class="dropdown-item" href="../profile/changepassword.php" style="margin-left: 30px; color: #eeeeee; background-color: #24282d">เปลี่ยนรหัสผ่าน</a>
              <a class="dropdown-item" href="../php/logout.php" style="margin-left: 30px; color: #eeeeee; background-color: #24282d">ออกจากระบบ</a>
              </a>
            </li>
          </ul>
        </div>
      </aside>

        <!-- End Main Sidebar -->
        <main class="main-content col-lg-10 col-md-9 col-sm-12 p-0 offset-lg-2 offset-md-3">
          <div class="main-navbar sticky-top" style="background-color: #24282d">

            <!-- <nav class="navbar align-items-stretch navbar-light flex-md-nowrap p-0"> -->
              <nav class="nav">
                <a href="#" class="nav-link nav-link-icon toggle-sidebar d-md-inline d-lg-none text-center border-left" data-toggle="collapse" data-target=".header-navbar" aria-expanded="false" aria-controls="header-navbar" >
                  <i class="material-icons">&#xE5D2;</i>
                </a>
              </nav>
            <!-- </nav> -->
          </div>
        <!-- / .main-navbar -->
        <div class="main-content-container container-fluid px-4">
          <!-- Page Header -->
          <div class="page-header row no-gutters py-4">
          </div>
          <div class="row">
            <!-- Users Stats -->
            <div class="col-lg-12 col-md-12 col-sm-12 mb-4">
              <div class="card card-small">

                <div class="card-body pt-0">
                  <div class="row border-bottom py-2" style="background-color: #24282d">
                    <span style="margin-left: 20px; color: #eeeeee">
                      <img src="../images/icon/pencil.png" width="25px" height="25px"/> &nbsp;
                       <span>เเก้ไขข้อมูลส่วนตัว</span></div>
                </div>
                <div class="card-header border-bottom">
                  <div class="custom-control custom-toggle custom-toggle-sm mb-1">

                      <div class="row">

                        <!-- edit picture -->

                        <div class="col-xl-5 col-lg-5 col-md-12 col-sm-12" style="text-align: center; padding-top: 10%; padding-left: 0px">

                          <div class="row">
                            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12" style="padding-left: 0px">
                              <img src="../images/icon/192x192.png">
                            </div>
                          </div>
                          <div class="row">
                              <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12" style="margin-top: 20px; padding-left: 0px; padding-bottom: 20px">
                                <button type="button" class="rounded text-white text-center p-1" data-toggle="modal" data-target="#changedata"
                                 style="background-color: #24282d; color: #eeeeee;">
                                 <img src="../images/icon/pencil.png" width="25px" height="25px"/> &nbsp;
                                  <span style="margin-left: 10px; margin-right: 10px">เปลี่ยนรูปภาพ</span>
                                </button>
                              </div>
                          </div>
                          <div class="row">
                            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12" style="padding-left: 0px">
                              
                            </div>
                          </div>

                        </div>

                        <!-- edit text -->

                        <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12" style="padding-left: 0px;">

                          <div class="row">

                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12" style="padding-left: 0px">
                              <strong class="d-block mb-2" style="color: #24282d">ชื่อ</strong>
                              <input type="text" class="form-control" id="update_name" name="update_name"
                              style="width: 100%; height: 40px; border-color: #24282d"/> </br>

                              <strong class="d-block mb-2" style="color: #24282d">นามสกุล</strong>
                              <input type="text" class="form-control" id="update_address" name="update_address"
                              style="width: 100%; height: 40px; border-color: #24282d"/> </br>

                              <strong class="d-block mb-2" style="color: #24282d">เบอร์โทร</strong>
                              <input type="text" class="form-control" id="update_province" name="update_province"
                              style="width: 100%; height: 40px; border-color: #24282d"/> </br>

                              <strong class="d-block mb-2" style="color: #24282d">ที่อยู่</strong>
                              <input type="text" class="form-control" id="update_tel" name="update_tel"
                              style="width: 100%; height: 144px; border-color: #24282d"/> </br>

                            </div>

                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12" style="padding-left: 0px">

                              <strong class="d-block mb-2" style="color: #24282d">ตำบล</strong>
                              <input type="text" class="form-control" id="update_surname" name="update_surname"
                              style="width: 100%; height: 40px; border-color: #24282d"/> </br>

                              <strong class="d-block mb-2" style="color: #24282d">อำเภอ</strong>
                              <input type="text" class="form-control" id="update_sdistrict" name="update_sdistrict"
                              style="width: 100%; height: 40px; border-color: #24282d"/> </br>

                              <strong class="d-block mb-2" style="color: #24282d">จังหวัด</strong>
                              <input type="text" class="form-control" id="update_district" name="update_district"
                              style="width: 100%; height: 40px; border-color: #24282d"/> </br>

                              <strong class="d-block mb-2" style="color: #24282d">LineID</strong>
                              <input type="text" class="form-control" id="update_line" name="update_line"
                              style="width: 100%; height: 40px; border-color: #24282d"/> </br>

                              <strong class="d-block mb-2" style="color: #24282d">Instagram</strong>
                              <input type="text" class="form-control" id="update_ig" name="update_ig"
                              style="width: 100%; height: 40px; border-color: #24282d"/> </br>

                            </div>

                            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12" style="padding-left: 0px; padding-bottom: 0px;">

                              <strong class="d-block mb-2" style="color: #24282d">E-mail</strong>
                              <input type="text" class="form-control" id="update_email" name="update_email"
                              style="width: 100%; height: 40px; border-color: #24282d"/> </br>

                            </div>
                          </div>

                        </div>

                      </div>

                    </div>             
                  </div>
      </main>
    </div>
  </div>

  <div class="row">
    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12" style="margin-top: 0px; text-align: right; padding-right: 60px;">
      <button type="button" class="btn" data-dismiss="modal"  style="background-color: #24282d">
        <span style="color: #eeeeee">ยกเลิก</span></button>
      <button type="submit" class="btn" style="background-color: #24282d; color: #eeeeee">บันทึก</button>
    </div>  
  </div>

  </from>

  <!-- popup -->
  <div class="modal fade" id="changedata" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header" style="background-color: #24282d">
          <h6 class="modal-title" id="exampleModalLabel" style="color: #eeeeee">เปลี่ยนรูปภาพ</h6>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
        </div>
        <div class="modal-body">
          <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 mb-4">
              <form action="" method="post" enctype="multipart/form-data">
                <input type="file" id="upload_file" name="upload_file" />
              </div>
            </div>
          </div>
          <div class="modal-footer" style="margin-top: 0px; text-align: right; padding-right: 30px; background-color: #24282d">
            <button type="button" class="btn btn-secondary" style="background-color: #eeeeee; color: #24282d" data-dismiss="modal">ยกเลิก</button>
            <button type="submit" class="btn btn-secondary" style="background-color: #eeeeee; color: #24282d">บันทึก</button>
          </div>
        </div>
      </div>
    </div>



    <script src="https://code.jquery.com/jquery-3.3.1.min.js" integrity="sha256-FgpCb/KJQlLNfOu91ta32o/NMZxltwRo8QtmkMRdAu8=" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.7.1/Chart.min.js"></script>
    <script src="https://unpkg.com/shards-ui@latest/dist/js/shards.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/Sharrre/2.0.1/jquery.sharrre.min.js"></script>
    <script src="../scripts/extras.1.0.0.min.js"></script>
    <script src="../scripts/shards-dashboards.1.0.0.min.js"></script>
    <script src="../scripts/app/app-blog-overview.1.0.0.js"></script>
</body>

</html>
